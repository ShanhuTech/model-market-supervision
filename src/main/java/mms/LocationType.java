...
@Module(description = "地点类型")
public final class LocationType extends AbstractModule {
    ...
    public LocationType(final HttpServlet httpServlet, final HttpServletRequest httpServletRequest, final HttpServletResponse httpServletResponse, final HashMap<String, Object> parameter) throws Exception {
    ...
    @Method(description = "添加地点类型", anonymousAccess = false, frequencys = {
    ...
    public final Message addLocationType() {
        try {
            ...
            // 添加地点类型
            {
                ...
                try {
                    ...
                    final mms.dao.LocationType obj = new mms.dao.LocationType(con);
                    final Message resultMsg = obj.addLocationType(name, description, order);
    ...
    @Method(description = "删除地点类型", anonymousAccess = false, frequencys = { @Frequency(source = Frequency.Source.IP, count = 10, unit = Frequency.Unit.SECOND) }, methodType = Method.Type.POST, parameters = {
        @Parameter(name = "uuid_array", text = "地点类型的uuid集合", type = Parameter.Type.STRING, allowNull = false, format = "^[0-9a-zA-Z]{1,40}(;[0-9a-zA-Z]{1,40})*$", formatPrompt = "以分号分割的1-40位的数字或大小写字母", remark = "") }, returns = @Returns())
    public final Message removeLocationType() {
        try {
            ...
            // 删除地点类型
            {
                ...
                try {
                    con = DruidInstance.getInstance().getTransConnection();
                    final mms.dao.LocationType obj = new mms.dao.LocationType(con);
                    final Message resultMsg = obj.removeLocationTypeByUuid(uuidArray);
                    this.messageResultHandler(resultMsg, con, true);
                    return resultMsg;
                } catch (final Exception e) {
                    return this.catchHandler(con, e);
    ...
    @Method(description = "修改地点类型", anonymousAccess = false, frequencys = {
        @Frequency(source = Frequency.Source.ACCOUNT, count = 10, unit = Frequency.Unit.SECOND) }, methodType = Method.Type.POST, parameters = {
            @Parameter(name = "uuid", text = "待修改地点类型的uuid", type = Parameter.Type.STRING, allowNull = false, format = "^[0-9a-zA-Z]{1,40}$", formatPrompt = "1-40位的数字或大小写字母", remark = ""),
    ...
    public final Message modifyLocationType() {
        try {
            ...
            // 修改地点类型
            {
                ...
                try {
                    con = DruidInstance.getInstance().getTransConnection();
                    final mms.dao.LocationType obj = new mms.dao.LocationType(con);
                    final Message resultMsg = obj.modifyLocationType(uuid, name, description, order);
                    this.messageResultHandler(resultMsg, con, true);
                    return resultMsg;
                } catch (final Exception e) {
                    return this.catchHandler(con, e);
    ...
    @Method(description = "获取地点类型", anonymousAccess = false, frequencys = {
        @Frequency(source = Frequency.Source.ACCOUNT, count = 10, unit = Frequency.Unit.SECOND) }, methodType = Method.Type.POST, parameters = {
            @Parameter(name = "uuid_array", text = "地点类型uuid的集合", type = Parameter.Type.STRING, allowNull = true, format = "^[0-9a-zA-Z]{1,40}(;[0-9a-zA-Z]{1,40})*$", formatPrompt = "以分号分割的1-40位的数字或大小写字母", remark = ""),
            ...
            @Parameter(name = "rows", text = "查询的行数", type = Parameter.Type.INTEGER, allowNull = true, format = "^[1-9]\\d*$", formatPrompt = "大于等于1的正整数", remark = "从1开始") }, returns = @Returns(results = {
                ...
                @ReturnResult(parentId = "array_id", id = "", name = "uuid", type = "string[1,40]", isNecessary = true, description = "地点类型的uuid"),
    ...
    public final Message getLocationType() {
        try {
            ...
            // 获取地点类型
            {
                ...
                try {
                    con = DruidInstance.getInstance().getTransConnection();
                    final mms.dao.LocationType obj = new mms.dao.LocationType(con);
                    final Message resultMsg = obj.getLocationType(uuidArray, nameArray, nameLike, orderArray, null, offset, rows);
                    this.messageResultHandler(resultMsg, con, true);
                    return resultMsg;
                } catch (final Exception e) {
                    return this.catchHandler(con, e);
