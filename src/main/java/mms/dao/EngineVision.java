 ...
 * 识别引擎
 */
public final class EngineVision extends AbstractDao {
    // 数据库表名
    public static final String DATABASE_TABLE_NAME = "mms_engine-vision";
    private Connection connection;
    private SimpleDateFormat simpleDateFormat;

    public EngineVision(final Connection connection) throws Exception {
     ...
     * @return 消息对象
     */
    public final Message addEngineVision(final String captureUuid, final String url, final String name, final String description, final String version) {
        try {
            // 数据检查
            {
                // 全部不允许为空
                this.allNotEmpty(new Object[] { captureUuid, url, name, version });
            }
            // 是否存在抓拍引擎
            {
            ...
            // 添加识别引擎
            {
                ...
                try {
                    ...
                    if (0 >= res) {
                        return new Message(Message.Status.ERROR, "ADD_ENGINE_VISION_FAIL", "添加识别引擎失败");
                    }
                } finally {
                    if (null != ps) {
                        ps.close();
     ...
     * 根据uuid删除识别引擎
     * 
     * @param uuidArray 识别引擎的uuid数组
     * @return 消息对象
     */
    public final Message removeEngineVisionByUuid(final String[] uuidArray) {
        try {
            // 数据检查
            {
                // 全部不允许为空
            ...
            // 删除识别引擎
            {
                ...
                try {
                    ...
                    final int res = ps.executeUpdate();
                    if (0 >= res) {
                        return new Message(Message.Status.ERROR, "REMOVE_ENGINE_VISION_FAIL", "删除识别引擎失败");
                    }
                } finally {
                    if (null != ps) {
                        ps.close();
     ...
     * @return 消息对象
     */
    public final Message modifyEngineVision(final String uuid, final String captureUuid, final String url, final String name, final String description, final String version) {
        try {
            // 数据检查
            {
                // 全部不允许为空
                ...
                // 至少一个不为空
                this.oneNotNull(new Object[] { captureUuid, url, name, description, version });
            }
            // 是否存在识别引擎
            {
                final Message resultMsg = this.getEngineVision(new String[] { uuid }, null, null, null, null, Integer.valueOf(0), Integer.valueOf(1));
                if (Message.Status.SUCCESS != resultMsg.getStatus()) {
                    return resultMsg;
                }
                final JSONArray array = ((JSONObject) resultMsg.getContent()).getJSONArray("array");
                if (0 >= array.length()) {
                    return new Message(Message.Status.ERROR, "ENGINE_VISION_NOT_EXIST", "识别引擎不存在");
                }
            }
            // 是否存在抓拍引擎
            {
            ...
            // 修改识别引擎
            {
                ...
                try {
                    ...
                    final int res = ps.executeUpdate();
                    if (0 >= res) {
                        return new Message(Message.Status.ERROR, "MODIFY_ENGINE_VISION_FAIL", "修改识别引擎失败");
                    }
                } finally {
                    if (null != ps) {
                        ps.close();
     ...
     * 获取识别引擎
     * 
     * @param uuidArray 识别引擎的uuid数组（允许为null）
     * @param captureUuidArray 抓拍引擎的uuid数组（允许为null）
     * @param nameArray 名称的数组（允许为null）
     * @param nameLike 名称的模糊查询（允许为null）
     * @param excludeUuidArray 排除识别引擎类型的uuid数组（允许为null）
     * @param offset 查询的偏移（允许为null）
     * @param rows 查询的行数（允许为null）
     * @return 消息对象
     */
    public final Message getEngineVision(final String[] uuidArray, final String[] captureUuidArray, final String[] nameArray, final String nameLike, final String[] excludeUuidArray, final Integer offset,
        final Integer rows) {
        try {
            final JSONArray whereArray = new JSONArray();
            if (null != uuidArray) {
            ...
            final String whereSql = DatabaseKit.composeWhereSql(whereArray);
            {
                ...
                ResultSet rs = null;
                try {
                    ps = this.connection.prepareStatement("select count(*) as `count` from `" + DATABASE_TABLE_NAME + "` v " + whereSql);
                    rs = ps.executeQuery();
                    if (rs.next()) {
            ...
            final JSONArray array = new JSONArray();
            {
                ...
                try {
                    ...
                    ps = this.connection.prepareStatement("select v.*, c.`name` as `capture_name` from `" + DATABASE_TABLE_NAME + "` v inner join `" + EngineCapture.DATABASE_TABLE_NAME
                        + "` c on v.`capture_uuid` = c.`uuid` " + whereSql + " order by v.`create_timestamp` desc " + limitCode);
                    if ((null != offset) && (null != rows)) {
