 ...
 * 组织架构类型
 */
public final class OrgType extends AbstractDao {
    // 数据库表名
    public static final String DATABASE_TABLE_NAME = "security_org-type";
    private Connection connection;
    private SimpleDateFormat simpleDateFormat;

    public OrgType(final Connection connection) throws Exception {
     ...
     * @return 消息对象
     */
    public final Message addOrgType(final String name, final String description, final Integer order) {
        try {
            // 数据检查
            {
                // 全部不允许为空
            ...
            // 是否存在组织架构类型名称
            {
                final Message resultMsg = this.getOrgType(null, new String[] { name }, null, null, null, Integer.valueOf(0), Integer.valueOf(1));
                if (Message.Status.SUCCESS != resultMsg.getStatus()) {
                    return resultMsg;
                }
                final JSONArray array = ((JSONObject) resultMsg.getContent()).getJSONArray("array");
                if (0 < array.length()) {
                    return new Message(Message.Status.ERROR, "ORG_TYPE_NAME_EXIST", "组织架构类型名称已存在");
                }
            }
            ...
            // 添加组织架构类型
            {
                ...
                try {
                    ...
                    if (0 >= res) {
                        return new Message(Message.Status.ERROR, "ADD_ORG_TYPE_FAIL", "添加组织架构类型失败");
                    }
                } finally {
                    if (null != ps) {
                        ps.close();
     ...
     * 根据uuid删除组织架构类型
     * 
     * @param uuidArray 组织架构类型的uuid数组
     * @return 消息对象
     */
    public final Message removeOrgTypeByUuid(final String[] uuidArray) {
        try {
            // 数据检查
            {
                // 全部不允许为空
            ...
            // 删除组织架构类型
            {
                ...
                try {
                    ...
                    final int res = ps.executeUpdate();
                    if (0 >= res) {
                        return new Message(Message.Status.ERROR, "REMOVE_ORG_TYPE_FAIL", "删除组织架构类型失败");
                    }
                } finally {
                    if (null != ps) {
                        ps.close();
     ...
     * 修改组织架构类型（至少修改一项字段）
     * 
     * @param uuid 组织架构类型的uuid
     * @param name 组织架构类型名称（允许为null）
     * @param description 描述（允许为空，为null不修改，长度为0则清空）
     * @param order 排序编号（允许为null）
     * @return 消息对象
     */
    public final Message modifyOrgType(final String uuid, final String name, final String description, final Integer order) {
        try {
            // 数据检查
            {
                // 全部不允许为空
            ...
            // 是否存在组织架构类型
            {
                // 是否存在组织架构类型
                {
                    final Message resultMsg = this.getOrgType(new String[] { uuid }, null, null, null, null, Integer.valueOf(0), Integer.valueOf(1));
                    if (Message.Status.SUCCESS != resultMsg.getStatus()) {
                        return resultMsg;
                    }
                    final JSONArray array = ((JSONObject) resultMsg.getContent()).getJSONArray("array");
                    if (0 >= array.length()) {
                        return new Message(Message.Status.ERROR, "ORG_TYPE_NOT_EXIST", "组织架构类型不存在");
                    }
                }
            }
            // 是否存在重名组织架构类型
            {
                if (null != name) {
                    // 是否存在组织架构类型名称
                    {
                        final Message resultMsg = this.getOrgType(null, new String[] { name }, null, null, new String[] { uuid }, Integer.valueOf(0), Integer.valueOf(1));
                        if (Message.Status.SUCCESS != resultMsg.getStatus()) {
                            return resultMsg;
                        }
                        final JSONArray array = ((JSONObject) resultMsg.getContent()).getJSONArray("array");
            ...
            // 修改组织架构类型
            {
                ...
                try {
                    ...
                    final int res = ps.executeUpdate();
                    if (0 >= res) {
                        return new Message(Message.Status.ERROR, "MODIFY_ORG_TYPE_FAIL", "修改组织架构类型失败");
                    }
                } finally {
                    if (null != ps) {
                        ps.close();
     ...
     * 获取组织架构类型
     * 
     * @param uuidArray 组织架构类型的uuid数组（允许为null）
     ...
     * @return 消息对象
     */
    public Message getOrgType(final String[] uuidArray, final String[] nameArray, final String nameLike, final Integer[] orderArray, final String[] excludeUuidArray, final Integer offset, final Integer rows) {
        try {
            final JSONArray whereArray = new JSONArray();
            if (null != uuidArray) {
                final JSONObject obj = new JSONObject();
